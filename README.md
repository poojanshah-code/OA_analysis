...
### `notebooks/05_type2_segmentation_classification.ipynb`
New dataset, new 3-class research task, built from a raw Google Drive folder rather than a
pre-split one:

* **Dataset** — Google Drive `Type2/` containing **exactly 3 class sub-folders** (names
  auto-detected, no pre-existing split required).
* **Split** — stratified **train 70% / val 15% / test 15%**, generated by the notebook.
* **Augmentation** — a medical-image recipe (flip, rotation, brightness/contrast, gamma, noise,
  CLAHE, elastic/grid distortion) applied to the training split only; before/after counts are
  reported as two tables (per class × train/val/test).
* **Segmentation** — trimmed to **4 classical methods** (Otsu+watershed, adaptive threshold,
  morphological active contour, GrabCut) **plus a newly trained U-Net** deep-segmentation model;
  the U-Net's final-ROI output is precomputed once and is what every classifier trains on.
* **16 classification architectures** — 2 each of ResNet / DenseNet / InceptionNet / MobileNet /
  EfficientNet / VGGNet, plus ViT-Base, Swin-Transformer, Swin+ResNet cross-attention and
  Swin+DenseNet cross-attention.

Produces the same publication-ready suite as the notebooks above (dataset-count tables,
segmentation collages, train/val tables, avg-metric tables+bars, class-wise bars+heatmap, learning
curves, confusion-matrix collage, confidence collages, Grad-CAM, morphological ANOVA/Kruskal-Wallis
stats, unfreeze sweep, a 7-step incremental ablation — raw → segmentation → augmentation → class
weighting → label smoothing → differential LR → cross-attention fusion — and a time-complexity
table), all saved as high-DPI PNG/CSV under `RESULTS_DIR`.
...
```
notebooks/   Colab notebooks (ablation, OA-HANet, testing, fine-tuning, Type2 3-class benchmark)
